...
export class Migration20260923102406_bookings extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(`create unique index "bookings_holidays_org_date_uq" on "bookings_holidays" ("tenant_id", "organization_id", "holiday_on") where "deleted_at" is null;`);
    this.addSql(`create index "bookings_holidays_org_tenant_date_idx" on "bookings_holidays" ("organization_id", "tenant_id", "holiday_on");`);
    ...
    this.addSql(`alter table "bookings_settings" add constraint "bookings_settings_tenant_org_uq" unique ("tenant_id", "organization_id");`);
    ...
    this.addSql(`create unique index "bookings_subject_categories_org_name_uq" on "bookings_subject_categories" ("tenant_id", "organization_id", lower("name")) where "deleted_at" is null;`);
    ...
    this.addSql(`create table "bookings_subjects" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "provider_key" text not null, "provider_record_id" text not null, "name" text not null, "category_id" uuid null, "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, primary key ("id"));`);
    this.addSql(`create unique index "bookings_subjects_provider_record_uq" on "bookings_subjects" ("tenant_id", "organization_id", "provider_key", "provider_record_id") where "deleted_at" is null;`);
    ...
    this.addSql(`create unique index "bookings_conflict_policy_exceptions_category_uq" on "bookings_conflict_policy_exceptions" ("tenant_id", "organization_id", "category_id") where "deleted_at" is null;`);
    ...
    this.addSql(`create index "bookings_bookings_org_window_idx" on "bookings_bookings" ("organization_id", "start_at", "end_at");`);
    ...
    this.addSql(`alter table "bookings_bookings" add constraint "bookings_bookings_duration_half_day_chk" check ("duration_working_days" > 0 and ("duration_working_days" * 2) = floor("duration_working_days" * 2));`);
    ...
    this.addSql(`create index "bookings_participants_booking_idx" on "bookings_participants" ("booking_id");`);
